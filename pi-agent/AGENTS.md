...
- PR/issue bodies are in my voice — first person, concrete ("I hit this on my Mac after the Node upgrade"), never third-person abstractions ("macOS users were hit"). A PR body is ≤ ~15 lines with exactly three parts, filling the repo's template headers if it has them and adding none:
  1. **What & why** — 1–3 sentences: the problem and what this does about it. Anything observable to others (metrics/log shape, behaviour change, migration, monitor that will move) goes here, first.
  2. **Non-obvious decision** — only if a reviewer would otherwise reverse-engineer it; one short paragraph. Link a design doc, don't inline one.
  3. **Verification** — exact commands and results (`rushx test src/sqs` → 30 suites, 378/378), the red-first test if there is one, any manual check. "Tests pass" is not evidence.
  Never: restating the diff file-by-file, Non-goals lists, precedent/PR citations, rebase narration, numbered § sections, tables, callouts. The reviewer has the diff; the body is intent + proof.
