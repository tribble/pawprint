// pr-review.ts: open_pr_review runs `pr-review --nvim <ref>` and hands it this session's
// intercom ID as the coordinator (pi-<sha256(session id)[0:32]>, the pi-subagents formula),
// never its name.
...
const opened = (coordinator: string) => ({
  code: 0,
  stdout: JSON.stringify({ ok: true, pr: 7, repo: "o/r", surface: "nvim", tab_id: "w:t2", pane_id: "w:p2", coordinator }),
  stderr: "",
});
...
test("open_pr_review: --coordinator is this session's intercom ID derived from the session id", async () => {
  ...
  assert.deepEqual(pi.execCalls, [["pr-review", "--nvim", "o/r#7", "--coordinator", MY_ID]]);
...
test("open_pr_review: an explicit coordinator (name or id) wins; focus:false adds --no-focus", async () => {
  ...
  assert.deepEqual(pi.execCalls, [["pr-review", "--nvim", "o/r#7", "--coordinator", "someone-else", "--no-focus"]]);
});

test("open_pr_review: pr-review failing surfaces its stderr", async () => {
  const { run } = openTool(async () => ({ code: 1, stdout: "", stderr: "pr-review: PR not found: o/r#7" }));
  await assert.rejects(run({ pr: "o/r#7" }), /pr-review --nvim failed: pr-review: PR not found: o\/r#7/);
});
